Page({
  data: {
    remoteFilePath:
      'https://fe-video-qc.xhscdn.com/fe-platform/541cb4b0e802f5bdd91845e85082132453f8d4f5.pdf',
    filePath: '',
    fileType: '',
  },
  onDownload() {
    this.task = xhs?.downloadFile({
      url: this.data.remoteFilePath, // 文件 ID
      success: res => {
        console.log('【downloadFile success】:', res);
        this.setData({ filePath: res.tempFilePath || res.filePath });
      },
      fail: res => {
  ...
  handleRemoteFilePath(e) {
    this.setData({ remoteFilePath: e.detail.value });
  },
  handleFilePath(e) {
